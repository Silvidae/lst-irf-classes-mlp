...
def main(args):
    ...
    path = args.path
    dir_name = f'{path}/rf_processed_{name}'
    ...
    # Train RF
    run(f"ictrainrf -i {dir_name}/gamma_part0_marked.h5 -o {dir_name}/psf_ -c ic_rf_std_config.json")
    
    # Apply RF
    run(f"icapplyrf -i {dir_name}/gamma_part1_marked.h5 "
        f"-r {dir_name}/psf_ic_rf.pkl "
        f"-o {dir_name}/ic_ "
        )
    run(f"icapplyrf -i {dir_name}/gamma_part1_marked.h5 "
        f"-r {dir_name}/psf_ic_rf.pkl "
    ...
    pyplot.savefig(f'{dir_name}/results_{name}/RF_matrix_{name}.png')


    # Apply RF to protons
    run(f"icapplyrf -i protons/dl2_simtel_corsika_theta_19.807_az_66.706_merged.h5 "
        f"-r {dir_name}/psf_ic_rf.pkl "
    ...
    for idx in range(1, n_classes + 1):
        run(f"lstchain_create_irf_files --overwrite "
            ...
            f"-o {dir_name}/irf_RF_class{idx}.fits "
            f"-c irf_dl3_tool_config.json")
    ...
    irfs = {
        evcls: load_irf_dict_from_file(f'{dir_name}/irf_RF_class{evcls}.fits')
    ...
    np.savez(
        f"{dir_name}/results_{name}/RF_sens_{name}.npz",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="RF Train and Apply Pipeline")
    parser.add_argument(
        '--proton_file', 
        type=str, 
        required=True, 
